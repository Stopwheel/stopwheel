...
if __name__ == "__main__":
    ...
    for i in range(3):
        for j in range(3):
            buttons[i][j] = Button(root, text="", font=("Arial", 20), height=2, width=5,
                                   command=lambda row=i, col=j: btn_click(buttons[row][col], row, col))
            buttons[i][j].grid(row=i, column=j, sticky=NSEW, pady=5, padx=5)
